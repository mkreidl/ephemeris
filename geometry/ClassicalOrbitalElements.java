...
public class ClassicalOrbitalElements extends OrbitalElements
{
    ...
    public void calculatePosition()
    {
        ...
        // Cartesian coordinates after inclination to the ecliptic
        // (rotation about the ascending node)
        double xi = xw;
        double yi = yw * cos( incl );
        double zi = yw * sin( incl );

        // Rotation within the ecliptic (new x/y-plane) to account for
        // longitude of the ascending node (N)
        eclipticalCartesian.x = cos( node ) * xi - sin( node ) * yi;
        eclipticalCartesian.y = sin( node ) * xi + cos( node ) * yi;
        eclipticalCartesian.z = zi;
