...
public class ModelPluto extends OrbitalModel
{
    ...
    {
        computeAll( time );
        position.set( posSpherical );
        velocity.set( velSpherical );
    }
    ...
    public void compute( Time time, Cartesian position, Cartesian velocity )
    {
        ...
        velocity.set( velCartesian );
    }

    private void computeAll( Time time )
    {
        ...
        velSpherical.lon = Math.toRadians( 0.00400703 + p1 * (
        ...
        velSpherical.lat = Math.toRadians( p1 * (
        ...
        velCartesian.x = ( velSpherical.dst * cl * cb - r * ( velSpherical.lon * sl * cb + velSpherical.lat * cl * sb ) ) / Time.SECONDS_PER_DAY;
        velCartesian.y = ( velSpherical.dst * sl * cb + r * ( velSpherical.lon * cl * cb - velSpherical.lat * sl * sb ) ) / Time.SECONDS_PER_DAY;
        velCartesian.z = ( velSpherical.dst * sb + r * velSpherical.lat * cb ) / Time.SECONDS_PER_DAY;
    }
}
